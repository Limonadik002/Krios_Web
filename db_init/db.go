...
func Migrate(db *sql.DB) error {
    ...
    BEGIN
        ...
        FOR i IN 1..60 LOOP
            ...
            -- Генерируем название (никогда не NULL)
            ...
            -- Генерируем цену (никогда не NULL)
            price_value := 100 + (i * 165) % 9900;
            
            -- Генерируем parametrs_name (всегда есть значение)
            parametrs_name_text := 'Характеристики товара ' || i::TEXT;
            
            -- Генерируем created_at (текущее время минус случайное количество дней)
            created_at_value := NOW() - (i || ' days')::INTERVAL;
            
            -- Генерируем JSONB характеристики (всегда не пустые)
            ...
            -- Вставка в objects (все поля заполнены)
            INSERT INTO objects (
                article, 
                name, 
                price, 
            ...
            ON CONFLICT (article) DO UPDATE SET
                ...
                parametrs_name = EXCLUDED.parametrs_name,
                characteristics = EXCLUDED.characteristics,
                version = objects.version + 1,
                created_at = EXCLUDED.created_at;
            
            -- Количество фото (1-5 штук)
            photo_count := 1 + (i % 5);
            
            -- Вставка фото (все поля заполнены)
